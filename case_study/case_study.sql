...
value ("Thuê theo năm"), ("Thuê theo tháng"), ("Thuê theo ngày"), ("Thuê theo giờ");
...
select kh.ho_va_ten, count(kh.ma_khach_hang) as "so_lan_dat_phong"
from khach_hang kh
join hop_dong dh on dh.ma_khach_hang = kh.ma_khach_hang
join loai_khach lk on lk.ma_loai_khach = kh.ma_loai_khach
where lk.ten_loai_khach like "Diamond"
group by kh.ma_khach_hang;

-- 5.Hiển thị ma_khach_hang, ho_ten, ten_loai_khach, ma_hop_dong, ten_dich_vu, ngay_lam_hop_dong, ngay_ket_thuc, tong_tien 
-- (Với tổng tiền được tính theo công thức như sau: Chi Phí Thuê + Số Lượng * Giá, với Số  	 và Giá là từ bảng dich_vu_di_kem, hop_dong_chi_tiet)
--  cho tất cả các khách hàng đã từng đặt phòng. (những khách hàng nào chưa từng đặt phòng cũng phải hiển thị ra).


select kh.ma_khach_hang, kh.ho_va_ten, lk.ten_loai_khach, hd.ma_hop_dong, dv.ten_dich_vu, hd.ngay_lam_hop_dong, hd.ngay_ket_thuc, sum( dv.chi_phi_thue + ifnull((hdct.so_luong * dvdk.gia),0)) as tong_tien
from khach_hang kh
...
select month(ngay_lam_hop_dong) as "thang" , count(*) as "so_luong_khach_hang"
from hop_dong
where year(ngay_lam_hop_dong) = 2021
...
select hd.ma_hop_dong, hd.ngay_lam_hop_dong, hd.ngay_ket_thuc, group_concat(dvdk.ten_dich_vu_di_kem),
hd.tien_dat_coc, ifnull(sum(so_luong),0) as so_luong_dich_vu_di_kem
from hop_dong hd
left join hop_dong_chi_tiet hdct on hdct.ma_hop_dong = hd.ma_hop_dong
...
select hd.ma_hop_dong, nv.ho_va_ten, kh.ho_va_ten, kh.so_dien_thoai, dv.ten_dich_vu, sum(ifnull(so_luong,0)) as so_luong_dich_vu_di_kem , hd.tien_dat_coc
from hop_dong hd
...
select nv.ma_nhan_vien, nv.ho_va_ten, td.ten_trinh_do, bp.ten_bo_phan, nv.so_dien_thoai, nv.dia_chi , count( hd.ma_nhan_vien) as so_lan_lap_hop_dong
from nhan_vien nv
