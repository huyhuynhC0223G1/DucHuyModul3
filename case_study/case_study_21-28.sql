...
where
        year (hd.ngay_lam_hop_dong) = 2021 and nv.dia_chi like "%Yên Bái%";

-- 22.Thông qua khung nhìn v_nhan_vien thực hiện cập nhật địa chỉ thành “Liên Chiểu” đối với tất cả các nhân viên được nhìn thấy bởi khung nhìn này.

update nhan_vien
set dia_chi = "Liên Chiểu"
where ma_nhan_vien = (select *
                      from (
                               select ma_nhan_vien
...
begin
    if
sp_ma_nhan_vien not in (select ma_nhan_vien from nhan_vien)
	then
        signal sqlstate '45000'
		set message_text = 'nhan vien khong ton tai';
end if;
        if
sp_ma_khach_hang not in (select ma_khach_hang from khach_hang)
	then
        signal sqlstate '45000'
		set message_text = 'khach hang khong ton tai';
end if;
        if
sp_ma_dich_vu not in (select ma_dich_vu from dich_vu)
	then
        signal sqlstate '45000'
